...
bool Image::Create(DeviceContext* device, const CreateParms_t& parms)
{
	...
	if (m_parms.format == VK_FORMAT_D32_SFLOAT)
	{
	...
	/** Allocate memory on GPU and attatch it to the image */
	...
	if (result != VK_SUCCESS)
	{
		printf("ËRROR: Failed to allocate memory!\n");
		assert(0);
		return false;
	}
...
{
	// Transition in the image layout
	...
	VkPipelineStageFlags srcSatge = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
	VkPipelineStageFlags dstSatge = VK_PIPELINE_STAGE_TRANSFER_BIT;
