...
inline Mat4 Quat::ToMat4() const
{
    ...
    Vec3 temp = RotatePoint({mat.rows[0].x, mat.rows[0].y, mat.rows[0].z});
    mat.rows[0].x = temp.x;
    mat.rows[0].y = temp.y;
    mat.rows[0].z = temp.z;
    temp = RotatePoint({mat.rows[1].x, mat.rows[1].y, mat.rows[1].z});
    mat.rows[1].x = temp.x;
    mat.rows[1].y = temp.y;
    mat.rows[1].z = temp.z;
    temp = RotatePoint({mat.rows[2].x, mat.rows[2].y, mat.rows[2].z});
    mat.rows[2].x = temp.x;
    mat.rows[2].y = temp.y;
    mat.rows[2].z = temp.z;
