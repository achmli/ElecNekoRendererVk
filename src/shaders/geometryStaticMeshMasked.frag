...
void main() {
    ...
    vec4 baseColorSample = vec4(mat.baseColor, mat.opacity);

    if (mat.baseColorTexId > -1) {
        vec4 baseColorTex = texture(textureArray, vec3(uv, mat.baseColorTexId));
        baseColorSample.rgb = baseColorTex.rgb;
        baseColorSample.a *= baseColorTex.a;
    }

    float alpha = clamp(baseColorSample.a, 0.0, 1.0);
